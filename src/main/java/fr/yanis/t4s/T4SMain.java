...
public class T4SMain implements ModInitializer {
    ...
    private static final String USER_AGENT = "Tip4Serv-Fabric/1.21.4";
    ...
    public static void checkConnection(Entity entity) {
        ...
        if (entity instanceof ServerPlayerEntity player) {
            if (connected) {
                player.sendMessage(Text.literal("Successfully connected to Tip4Serv API"));
            } else {
                player.sendMessage(Text.literal("Could not connect to Tip4Serv API."));
            }
        } else {
            if (connected) {
