...
public class T4SMain implements ModInitializer {
    ...
    public void onInitialize() {
        LOGGER.info("[Tip4Serv] Initializing mod (Fabric 1.21.4).");
        INSTANCE = this;
            ...
            try {
                ...
                for (int i1 = 0; i1 < infosArr.size(); i1++) {
                    ...
                    if (redo_cmd) {
                        ...
                        if (player != null) player.sendMessage(Text.literal(Tip4ServConfig.getMessageSuccess()));
                    }
                ...
                writeResponseFileAsync(lastResponse).thenRun(() -> {
                    if (finalUpdateNow) sendResponse();
                });
            } catch (Exception e) {
    ...
    public static void sendResponse() {
        if (Tip4ServKey.getApiKey().isEmpty() || Tip4ServKey.getServerID().isEmpty() || Tip4ServKey.getPrivateKey().isEmpty()) return;
        try {
    ...
    public static String sendHttpRequest(String cmd) {
        if (Tip4ServKey.getApiKey().isEmpty() || Tip4ServKey.getServerID().isEmpty() || Tip4ServKey.getPrivateKey().isEmpty()) return "false";
        try {
            long timestamp = System.currentTimeMillis();
            String fileContent = readResponseFile();
            String jsonEncoded = URLEncoder.encode(fileContent.isEmpty() ? "{}" : fileContent, StandardCharsets.UTF_8);
            ...
            if ("update".equals(cmd)) clearResponseFile();
    ...
    public static void checkConnection(Entity entity) {
        if (Tip4ServKey.getApiKey().isEmpty() || Tip4ServKey.getServerID().isEmpty() || Tip4ServKey.getPrivateKey().isEmpty() || Tip4ServKey.getPublicKey().isEmpty()) {
            if (entity == null) {
                ...
                player.sendMessage(Text.literal("Please provide a correct apiKey in tip4serv/tip4serv.key file"));
            }
            return;
        }
        ...
        if (entity == null) {
            ...
            if (response.contains("[Tip4serv error]")) {
                player.sendMessage(Text.literal("Error while connecting to Tip4Serv API: " + response));
            } else {
                player.sendMessage(Text.literal("Connection to Tip4Serv API: " + response));
            }
        }
    }
    ...
    private static String readResponseFile() {
        try {
            ...
            return file.exists() ? Files.readString(Paths.get(RESPONSE_FILE_PATH)) : "";
        } catch (Exception e) {
            return "";
        }
    }
